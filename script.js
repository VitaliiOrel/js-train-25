...
//Завдання 4
function postDataWithAuth(url, data, authToken) {
  try {
    fetch(url, {
      method: "POST",
...
async function* gatherData() {
  try {
    ...
    const casheData = await getDataFromCache()
    yield casheData
...
async function displayData() {
  ...
  gData.next().then(result => console.log(result.value))
  gData.next().then(result => console.log(result.value))
  gData.next().then(result => console.log(result.value))
...
while (true) {
  let nextCount = countdown.next()
  console.log(nextCount.value)
  if (nextCount.done) {break}
}
